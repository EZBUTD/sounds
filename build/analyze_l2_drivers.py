#!/usr/bin/env python3
"""
Do unusual consonant and vowel inventories deter learners?
...
Conclusion for the page: this small, selected roster shows no negative
association between the rarity measure and learner estimates. Social reach is
more closely associated with learner numbers here, but these descriptive
correlations do not establish causes.
...
CAVEATS
  ...
  - Lingua-franca status is a hand-coded binary (documented below), which is a
    ...
    history-based variable aligns more closely than this phonetic measure, not to
    quantify empire.
  ...
  - THE RARITY AXIS IS SEGMENTAL ONLY. It scores consonants and vowels. Tone is
    ...
    tone contrasts its selected source records separately. Read a low score as
    "few unusual consonants and vowels in this source", never as "easy".
...
def main():
    ...
    print("=" * 74)
    print(f"DO UNUSUAL CONSONANTS/VOWELS DETER LEARNERS?  (n = {n} with L2 data)")
    print("=" * 74)
    ...
    print("more sounds has more chances to include unusual ones. Hold size steadier and")
    print(f"the association becomes {p_size:+.3f}. Meanwhile lingua-franca status")
    print(f"correlates with learners at {r_lfl:+.3f}; its sample correlation with rarity is")
    print(f"({r_lfr:+.3f}). In this roster, learner numbers align more closely with")
    print("social reach than with this sound-rarity measure; this does not show cause.")
