#!/usr/bin/env python3
"""Build prototype/history_data.js — the English sound-flow diagram + timeline.
...
published histories (cited on the page).
...
OUT = ROOT / "prototype" / "history_data.js"
...
FLOWS = [
    ...
    },
    {
        ...
        "srcRoster": "French",
        "srcFate": "has",
        "how": "Usually called English's most recent addition — from French "
               "loans, and from {{z}} plus {{j}} collapsing together, the process "
               "that still turns <em>educate</em> into <em>ejucate</em> in casual "
               "speech. It never got a letter of its own.",
...
EVENTS = [
    ...
    },
    {
        ...
        "body": "French becomes the language of court, and a large layer of "
                "vocabulary follows. {{v}}, {{z}} and {{dʒ}} all owe their "
                "standing as separate English sounds to it.",
    },
    {
        "when": "about 1400 to 1700",
        "kind": "shifted",
        "title": "The Great Vowel Shift",
        "body": "The long vowels move, mostly upward. <em>Name</em> had the {{a}} "
                "of <em>father</em>; <em>see</em> had the {{e}} of <em>they</em>; "
                "<em>mine</em> had {{i}}. The highest had nowhere to go and broke "
                "into diphthongs.",
    },
    {
        "when": "from 1476",
        "kind": "shifted",
        "title": "Printing fixes the spelling mid-change",
        "body": "Caxton sets up England's first press at Westminster. Spelling "
                ...
                "lost. The spellings frozen were the ones current then.",
    },
    {
...
def load_data_js():
    src = (ROOT / "prototype" / "data.js").read_text(encoding="utf-8")
    m = re.search(r"const DATA\s*=\s*(\{.*\});?\s*$", src.strip(), re.S)
    if not m:
        m = re.search(r"=\s*(\{.*\})\s*;?\s*$", src.strip(), re.S)
    if not m:
        sys.exit("build_history_data: could not parse prototype/data.js")
...
def main():
    ...
    OUT.write_text(
        ...
        "// Every branch end is verified against prototype/data.js at build time\n"
        "// and re-verified in the browser by smoke_pages.mjs.\n"
